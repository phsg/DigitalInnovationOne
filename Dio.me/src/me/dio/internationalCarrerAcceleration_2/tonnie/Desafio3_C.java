...
public class Desafio3_C {
	...
	private static int diffent(String[] pArrayPlay_1, String[] pArrayPlay_2) {
		...
		int y = 0;
		int vUltimaCarta = 0;
		for (x = 0; x < pArrayPlay_1.length;) {
			int vCard_1 = Integer.parseInt(pArrayPlay_1[x]);
			if (vP == pArrayPlay_2.length) {
				vResult++;
				x++;
			} else {
				for (y = vP; y < pArrayPlay_2.length;) {
					int vCard_2 = Integer.parseInt(pArrayPlay_2[y]);
					// Carta menor
					if (vCard_1 < vCard_2) {
						if (vCard_1 != vUltimaCarta) {
							vResult++;
							vUltimaCarta = vCard_1;
						}
						x++;
						break;
					}
...
//		String vPlay_1 = "1 2 3";
//		String vPlay_2 = "1 2 3";
...
//		System.out.println(diffent(vArrayPlay_2, vArrayPlay_1));
		int vTest_1 = (diffent(vArrayPlay_1, vArrayPlay_2));
		int vTest_2 = (diffent(vArrayPlay_2, vArrayPlay_1));

		if (vTest_1 < vTest_2) {
			System.out.println(vTest_1);
		} else {
			System.out.println(vTest_2);
		}
